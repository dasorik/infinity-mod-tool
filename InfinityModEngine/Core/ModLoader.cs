...
namespace InfinityModEngine
{
	public class ModLoader
	{
		...
		public IEnumerable<ModLoadResult> LoadMods(params string[] modsToLoad)
		{
			DeleteAndRecreateFolder(configuration.CacheFolder);
			...
			foreach (var modPath in modsToLoad)
			{
				...
				var loadResult = TryLoadV2Mod(fileInfo);

				if (loadResult.status == ModLoadStatus.Success && results.Any(r => loadResult.modData.ModID == r.modData?.ModID))
					loadResult = new ModLoadResult(loadResult.modFileName, loadResult.modData, ModLoadStatus.DuplicateID, loadResult.loadErrors);
		...
		private ModLoadResult TryLoadV2Mod(FileInfo fileInfo)
		{
			BaseModConfiguration modData = null;
			string zipFile = null;
			...
			try
			{
				...
				{
					var extractFolder = Path.Combine(configuration.CacheFolder, fileInfo.Name);
					...
					if (!File.Exists(configPath))
						return new ModLoadResult(fileInfo.Name, modData, ModLoadStatus.NoConfig);
					...
					if (string.IsNullOrEmpty(modData.ModID))
						return new ModLoadResult(fileInfo.Name, modData, ModLoadStatus.ConfigInvalid);

					if (modData.Version < 2)
						return new ModLoadResult(fileInfo.Name, modData, ModLoadStatus.UnsupportedVersion);

					if (modData.Version > 2)
						return new ModLoadResult(fileInfo.Name, modData, ModLoadStatus.UnsupportedVersion);

					// Temporary work around since we can't appear to load images from %APPDATA%
					var imageFileInfo = new FileInfo(Path.Join(extractFolder, modData.DisplayImage));
					...
					modData.CacheFolderName = fileInfo.Name;

					var loadErrors = new List<string>();
					var configValid = CheckModConfigurationIsValid(modData, loadErrors);

					return new ModLoadResult(fileInfo.Name, modData, configValid ? ModLoadStatus.Success : ModLoadStatus.ConfigInvalid, loadErrors);
				}
				else
				{
			...
			catch (Exception ex)
			{
				...
				return new ModLoadResult(fileInfo.Name, modData, ModLoadStatus.UnspecifiedFailure);
			}
			finally
			{
