...
namespace InfinityModFramework.Utilities
{
	public class ModInstaller
	{
		...
		public ModInstaller(Configuration configuration, IGameIntegration integration, ILogger logger = null)
		{
		...
		public async Task<InstallationStatus> ApplyChanges(ModInstallationInfo[] mods, bool ignoreWarnings)
		{
			return await ApplyChangesInternal(mods, ignoreWarnings, false);
		}

		private async Task<InstallationStatus> ApplyChangesInternal(ModInstallationInfo[] mods, bool ignoreWarnings, bool reverting)
		{
			CreateTempFolder();

			reservedFiles = GetReservedFiles();
			autoMappings = integration.GetQuickBMSAutomappings();
		...
		{
			string[] reservedFiles = integration.GetReservedFiles();
		...
		public static string ResolvePath(string path, ModInstallationInfo mod, Configuration config)
		{
		...
		public static string ResolveModPath(string path, ModInstallationInfo mod)
		{
		...
		private bool HasAutoMapping(string resourceFile, Configuration config, out QuickBMSAutoMapping autoMapping)
		{
			...
			{
				var files = Directory.GetFiles(ModInstaller.ResolveGamePath(map.TargetDirectory, config));
